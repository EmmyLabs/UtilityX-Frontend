const FEATURES = [
  {
    ...
    iconBg: "bg-[#5B3DF5]/20",
    title: "Secure Escrow",
    desc: "100% protection for buyers and sellers.",
  },
  {
    icon: "diamond",
    iconColor: "text-[#22E6B8]",
    iconBg: "bg-[#22E6B8]/20",
    title: "Best Prices",
    desc: "Competitive rates across all networks.",
  },
  {
    icon: "bolt",
    ...
    desc: "Airtime, data & gift cards delivered instantly.",
  },
];
...
export default function AuthBrandPanel() {
  return (
    /*
     * mobile (<768px): hidden — right panel takes full width
     * tablet (768px+): 50% / 45% width, h-full fills the flex row
     * overflow-hidden clips the glow blobs and wallet illustration
     */
    <aside className="hidden md:flex flex-col md:w-1/2 lg:w-[45%] h-full relative overflow-hidden bg-[#0B1020] px-8 lg:px-12 pt-8 pb-8">
      ...
      <div className="relative z-10 flex items-center gap-3 shrink-0">
        <div className="w-10 h-10 rounded-full bg-[#5B3DF5]/20 border border-[#5B3DF5]/40 flex items-center justify-center shrink-0">
          <span className="material-symbols-outlined text-[#5B3DF5] text-[22px]">bolt</span>
        </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center gap-5 py-6 min-h-0">
        ...
        <ul className="space-y-3">
          {FEATURES.map((f) => (
            <li key={f.title} className="flex items-start gap-3">
              <div className={`w-9 h-9 rounded-lg ${f.iconBg} flex items-center justify-center shrink-0 mt-0.5`}>
                <span className={`material-symbols-outlined ${f.iconColor} text-[20px]`}>{f.icon}</span>
              </div>
              <div>
        ...
        {/* Wallet illustration — max-height constrained so it never overflows */}
