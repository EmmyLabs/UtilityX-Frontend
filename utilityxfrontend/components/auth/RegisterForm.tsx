...
import { register } from "@/lib/mock/authService";
...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    const res = await register(phone.trim(), password);
    setLoading(false);
    ...
    // Store phone in context so OTP screen knows whose code to verify
    ...
    <div className="w-full max-w-[480px] mx-auto flex flex-col py-4">
      ...
      <form className="space-y-5" onSubmit={handleSubmit} noValidate>
        ...
        <div className="grid grid-cols-2 gap-3">
          <SocialButton icon={<GoogleIcon />} label="Continue with Google" type="button" className="h-14" />
          <SocialButton icon={<AppleIcon />}  label="Continue with Apple"  type="button" className="h-14" />
        </div>
      </form>
